public class Menu
{
    ...
    {
        int choice = 0;
        List<StayHistory> histories = _stayManager.GetList(Customer.LoggedInCustomerId);
        ...
        {
            Console.WriteLine("Choose which booking to increase stay period: ");
            foreach (StayHistory history in histories)
            {
                Console.WriteLine($"{++choice}. {history.RoomType} {history.RoomNumber}");
            }
            choice = int.Parse(Console.ReadLine());
        }
        ...
        TimeSpan time = new TimeSpan(histories[choice].StayPeriod, 0, 0, 0, 0);
        histories[choice].CheckOutDate = histories[choice].CheckInDate.Add(time);
