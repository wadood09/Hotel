...
using My_Dapper_Project_2.Services.Implementation;
    ...
    public class CustomerMenu
    {
        ...
        private void BookARoom()
        {
            ...
            else
            {
                ...
                Console.WriteLine($"Your room number is {room.RoomNumber}");
                ...
                _bookingService.CreateBooking(hotel.Name, hotel.Id, type.Name, type.Id, hasRoomService, roomService, room.RoomNumber, room.Id, status, nights, period, price, paid);
                Read();
            }
        }

        private void CheckWallet(int roomTypeId, int nights, RoomService? service, out double price)
        {
        ...
        private void RoomTypeSection(out RoomType? type, int hotelId)
        {
        ...
        private void RoomSection(out int nights, out DatePeriod period, out Room? room, int roomTypeId, ref char choice)
        {
            int checkInDate = 0;
        ...
        private void RoomServiceSection(ref RoomService? roomService, ref bool hasRoomService, int hotelId)
        {
            char choice = 'Y';
            if (skip) goto Skip;
            Console.WriteLine("Do you want hotel to provide you additional room service (Y/N): ");
            generalMenu.EnterChoice(ref choice);
        ...
        private void IncreaseStayPeriod()
        {
            ...
            if (shouldIncrease)
            {
                ...
                bookings[choice].CheckOutDate.AddDays(days);
                bookings[choice].Nights += days;
                _bookingService.Update(bookings[choice]);
                Console.WriteLine("Successfully increased stay period!!!");
            }
            else
            {
                Console.WriteLine($"Room booked is not available on {bookings[choice].CheckOutDate.AddDays(days): dddd, dd mmm, yyyy}!!!");
        ...
        private void ChangeCheckInTime()
        {
            ...
            DatePeriod stayPeriod = new(bookings[choice].CheckInDate, bookings[choice].CheckOutDate);
            if (stayPeriod.WithInRange(DateTime.Today))
            {
            ...
            else
            {
                ...
                bool ShouldChange = _bookingService.ShouldChangeCheckInTime(days, bookings[choice], out DateTime newCheckInDate, out DateTime newCheckOutDate);
                if (ShouldChange)
                {
                    bookings[choice].CheckInDate = newCheckInDate;
                    bookings[choice].CheckOutDate = newCheckOutDate;
                    _bookingService.Update(bookings[choice]);
                    Console.WriteLine($"Successfully changed Checkin time!!!");
                }
                else
                {
                    if (DateTime.Today.AddDays(days) > bookings[choice].CheckOutDate)
        ...
        private void ViewRoomDetails()
        {
            ...
            Console.ForegroundColor = colours[random.Next(0, colours.Length)];
            Console.WriteLine(bookings[choice].CheckInDate.ToString("dddd, MMMM dd, yyyy"));
            Console.ForegroundColor = ConsoleColor.Gray;

            Console.Write("Time to be checked-out: ");
            Console.ForegroundColor = colours[random.Next(0, colours.Length)];
            Console.WriteLine(bookings[choice].CheckOutDate.ToString("dddd, MMMM dd, yyyy"));
            Console.ForegroundColor = ConsoleColor.Gray;
            Read();
        }
        ...
        private void CreditWallet(int hotelId, double price)
        {
